...
LOAD_LOG_TABLE = 'RAW_TRADES_LOAD_LOG'
...
cs.execute(f"SELECT source_file FROM {SCHEMA}.{LOAD_LOG_TABLE}")
...
for root, dirs, files in os.walk(file_path):
    for filename in files:
        if filename.lower().endswith(".xlsx") and filename not in loaded_files:
            ...
            df = df[['ID', 'TRADE_TYPE', 'TRADE_TIME', 'TRADE_COMMENT',
...
if all_dfs:
    ...
    if success:
        ...
        # Update load log
        for filename in set(combined_df['SOURCE_FILE']):
            cs.execute(f"""
                INSERT INTO {SCHEMA}.{LOAD_LOG_TABLE}(source_file, load_timestamp)
                VALUES ('{filename}', CURRENT_TIMESTAMP)
            """)
        print("Load log updated.")
    else:
